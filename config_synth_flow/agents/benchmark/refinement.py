...
class EditAgent(BaseAgent):
    ...
    async def run_agent(
        ...
        try:
            ...
            return self.get_response_item(resp)
        except Exception as e:
            self.logger.error(f"{e}. ")
            return QueryResponseItem(query="", response="")

    def get_response_item(self, response: str) -> QueryResponseItem:
        if "</think>" in response:
            response = response.split("</think>")[-1].strip()
        response += "\n"
        response = re.sub(r"```.*\n", "", response)
        return QueryResponseItem(**json.loads(response))
...
class ValidAgent(BaseAgent):
    ...
    async def run_agent(
        ...
        for _ in range(3):
            try:
                resp = await self.chat(prompt)
                resp = self.get_response_item(resp)
                return resp
            except Exception as e:
                self.logger.error(f"{e}. {resp}")
                continue
        return ValidResponseItem(
            reason="模型無法回應", suggestion="模型無法回應", score=0
        )

    def get_response_item(self, response: str) -> ValidResponseItem:

        if "</think>" in response:
            think = re.search(
                r"<think>(.*)</think>", response, re.DOTALL | re.MULTILINE
            ).group(1)
            response = response.split("</think>")[-1].strip()
        else:
            think = ""
...
class QARefinementAgent(BaseAgent):
    ...
    async def run_agent(self, messages: list[dict[str, str]], dct: dict) -> dict:
        ...
        best_mes, best_score = messages, -1
        for i in range(self.max_rounds):
            ...
            history.append({"role": "validation", "content": valid_res.model_dump()})
            if valid_res.score > best_score:
                best_score = valid_res.score
                best_mes = messages
            if self.early_stop_criteria_lambda(valid_res):
                break
            
        return {"history": history, "messages": best_mes}
