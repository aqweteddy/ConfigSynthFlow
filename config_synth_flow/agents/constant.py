DEFAULT_EDIT_QA_TEMPLATE = """### 改善對話

你將會獲得一段 **user** 和 **assistant** 的聊天紀錄，目的是基於先前的評估結果，針對對話進行最佳化，確保符合高品質標準。
...
DEFAULT_VALID_QA_TEMPLATE = """### 任務說明
...
1. **問題品質（Question Quality）：**
   - 問題是否清晰、具體，並聚焦於以下主題之一：
     ...
     - 查詢特定產品或編號細節
     - 合規性檢查
   - 是否避免模糊或過於寬泛的提問。問題應包含關鍵字或具體實體（如產品名稱、數字、編號）。
