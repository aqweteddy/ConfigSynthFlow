...
class MagpieBasedOnTextAgent(BaseAgent):
    def __post_init__(
        ...
        super().__post_init__(model, openai_kwargs, gen_kwargs, output_col=output_col)
    ...
    async def chat(self, messages: list[dict[str, str]], role: str) -> str:
        ...
        for i in range(5):
            resp: Completion = await self.openai_client.completions.create(prompt=prompt, **self.gen_kwargs)
            result = []
    ...
    async def run_agent(self, dct: dict) -> list[dict[str, str]]:
        ...
        for _ in range(self.num_max_turns):
            ...
            if not user:
                break
            assistant = await self.chat(messages + [{'role': 'user', 'content': user}], 'assistant')
            # print(assistant, '\n\n\n-------')
            if not assistant:
                break
